<template>
    <div class="icon fill">
        <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
            <g>
                <path class="st0"
                      d="M245.6,85.5v176.1l-74.9,74.9c-4.3,4.3-4.3,11.3,0,15.6c2.2,2.2,5,3.2,7.8,3.2c2.8,0,5.7-1.1,7.8-3.2   l78.2-78.2c1-1,1.9-2.3,2.4-3.6c0-0.1,0-0.1,0.1-0.2c0.5-1.3,0.8-2.6,0.8-4V85.5c0-6.1-5-11.1-11.1-11.1S245.6,79.4,245.6,85.5z"/>
                <path class="st0"
                      d="M256.7,1C115.9,1,1.3,115.5,1.3,256.3s114.5,255.3,255.3,255.3c64.3,0,125.8-24,173.1-67.6   c4.5-4.1,4.8-11.1,0.6-15.6c-4.1-4.5-11.1-4.8-15.6-0.6c-43.2,39.8-99.3,61.8-158.1,61.8c-128.6,0-233.2-104.6-233.2-233.2   S128.1,23.1,256.7,23.1s233.2,104.6,233.2,233.2c0,43.6-12.1,86.1-35,122.9c-3.2,5.2-1.6,12,3.6,15.2c5.2,3.2,12,1.6,15.2-3.6   C498.8,350.6,512,304,512,256.3C512,115.5,397.5,1,256.7,1z"/>
            </g>
        </svg>
    </div>
</template>

<script>
export default {
    name: "clock-icon",
};
</script>

<style lang="scss" scoped>
.icon {
    @include flex-center;
    width: 22px;
    height: 22px;
}

.fill svg * {
    fill: black;
}
</style>